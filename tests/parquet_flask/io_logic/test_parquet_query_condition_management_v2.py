...
class TestGeneralUtilsV2(unittest.TestCase):
    def test_time_range_01(self):
        ...
        condition_manager = ParquetQueryConditionManagementV2('s3a://mock-bucket/base-path/', -99999, props)
        ...
        self.assertEqual(1, len(condition_manager.parquet_names), f'wrong parquet names list length')
        for each_parquet_name in condition_manager.parquet_names:
    ...
    def test_time_range_02(self):
        ...
        self.assertEqual(13, len(condition_manager.parquet_names), f'wrong parquet names list length')
        for each_parquet_name in condition_manager.parquet_names:
            self.assertTrue(each_parquet_name in expected_parquet_names, f'missing in expected_parquet_names: {each_parquet_name}')
        self.assertEqual(condition_manager.conditions, expected_conditions, f'wrong conditions')
        self.assertEqual(condition_manager.columns, expected_columns, f'wrong __columns')
    ...
    def test_time_range_03(self):
        ...
        self.assertEqual(14, len(condition_manager.parquet_names), f'wrong parquet names list length')
        for each_parquet_name in condition_manager.parquet_names:
            self.assertTrue(each_parquet_name in expected_parquet_names, f'missing in expected_parquet_names: {each_parquet_name}')
        self.assertEqual(condition_manager.conditions, expected_conditions, f'wrong conditions')
        self.assertEqual(condition_manager.columns, expected_columns, f'wrong __columns')
    ...
    def test_time_range_04(self):
        ...
        self.assertEqual(6, len(condition_manager.parquet_names), f'wrong parquet names list length')
        for each_parquet_name in condition_manager.parquet_names:
            self.assertTrue(each_parquet_name in expected_parquet_names, f'missing in expected_parquet_names: {each_parquet_name}')
        self.assertEqual(condition_manager.conditions, expected_conditions, f'wrong conditions')
        self.assertEqual(condition_manager.columns, expected_columns, f'wrong __columns')
    ...
    def test_time_range_05(self):
        ...
        for each in range(3, 13):
            expected_parquet_names.append(f's3a://mock-bucket/base-path/provider=mock_provider/project=mock_project/platform_code=123/year=2018/month={each}')
        self.assertEqual(11, len(condition_manager.parquet_names), f'wrong parquet names list length')
        for each_parquet_name in condition_manager.parquet_names:
            self.assertTrue(each_parquet_name in expected_parquet_names, f'missing in expected_parquet_names: {each_parquet_name}')
        self.assertEqual(condition_manager.conditions, expected_conditions, f'wrong conditions')
        self.assertEqual(condition_manager.columns, expected_columns, f'wrong __columns')
